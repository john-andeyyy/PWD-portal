...
import { Accessibility, BriefcaseBusiness, LayoutDashboard, LogOut, UserCircle2, Users } from 'lucide-react';
import type { ReactNode } from 'react';

interface SidebarItem {
    key: 'dashboard' | 'members' | 'accounts' | 'profile';
    label: string;
    href: '/dashboard' | '/members' | '/accounts' | '/profile';
    icon: ReactNode;
}
            ...
            {sidebarItems.map((item) => {
                ...
                return (
                    <Link
                        ...
                        className={`flex min-h-11 w-full items-center gap-3 rounded-lg px-3 py-2.5 text-left text-sm font-medium transition ${isActive
...
export function VersionBlock({ onLogout }: SidebarProps) {
    return (
        <div className="border-t border-slate-200 pt-4 dark:border-slate-800">
            ...
            <button
                type="button"
                onClick={onLogout}
                className="flex min-h-11 w-full items-center justify-center gap-2 rounded-lg px-3 py-2.5 text-sm font-medium text-rose-600 transition hover:bg-rose-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-rose-400 dark:text-rose-400 dark:hover:bg-rose-950/50"
            >
                <LogOut size={16} />
                Logout
            </button>
...
export function Sidebar({ onLogout }: SidebarProps) {
    return (
        <aside className="hidden w-72 shrink-0 border-r border-slate-200 bg-white px-4 py-5 dark:border-slate-800 dark:bg-slate-950 lg:fixed lg:inset-y-0 lg:left-0 lg:flex lg:flex-col">
            ...
            <div className="mt-6"><VersionBlock onLogout={onLogout} /></div>
        </aside>
    );
}
